...
void HandleClients(int clntSocket, int *logs, int *queue1, int *queue2) {
...
void HandleCashier1(int clntSocket, int *logs, int *queue1) {
    char echoBuffer[RCVBUFSIZE];
    int recvMsgSize;
    char echoString[30]; 
...
void HandleCashier2(int clntSocket, int *logs, int *queue2) {
    char echoBuffer[RCVBUFSIZE];
    int recvMsgSize;
    char echoString[30]; 
    unsigned int echoStringLen;
...
void HandleListener(int clntSocket, int *logs) {
    char echoString[50]; 
    unsigned int echoStringLen;
...
void handle(int clntSocket, int *logs, int *queue1, int *queue2) {
     char echoBuffer[RCVBUFSIZE];
    int recvMsgSize;
    ...
    if (echoBuffer[0] == '0') {
        HandleClients(clntSocket, logs, queue1, queue2);
    } else if (echoBuffer[0] == '1') {
        HandleCashier1(clntSocket, logs, queue1);
    } else if (echoBuffer[0] == '2') {
        HandleCashier2(clntSocket, logs, queue2);
    } else if (echoBuffer[0] == '3') {
        HandleListener(clntSocket, logs);
    }
}